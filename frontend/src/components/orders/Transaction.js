...
export const TaxTable = ({taxes}) => {
    return (
        <table className='table w-100 tax-table-receipt' style={{width:'100%'}}>
...
const Transaction = ({isLoading, orderProducts, taxes, order, total }) => {
    ...
    const showQT = (products) => {
        ...
        products.forEach( k => {
            let {stock, price} = k
            if(price > 0) { // if price is negative then it was returning product (not to be considered in overall products)
                // if(Number(stock) < ((stock-0)+1)) { // agar koi product 0.2 ya 1 se km h to usko 1 maano bs
        ...
        <>
            ...
            <div className='desc row' style={{textAlign:'right', justifyContent:'right'}}>
                <div style={{lineHeight:1,marginTop:10}}> 
                    <span style={{cssText:"font-size:1.3rem!important;font-weight:600;margin-top:5px"}} className='total-cost'> 
                    ...
                    <div style={{cssText:"font-size:1rem;font-weight:600"}}>
                        {order.modes ? Object.keys(order.modes).map( (m,i) => {
                            if(order.modes[m] && m!=='Cash') {
                                return <div key={i} style={{cssText:"font-size:1rem;font-weight:600"}}>
                    ...
                    {order.r && (<><div style={{cssText:"font-size:1rem;font-weight:600"}}>{order.r}</div></>)}
                    <div style={{cssText:"font-size:1rem;font-weight:600;"}}>Products: {showQT(orderProducts)}</div>
                </div>
            </div>
            <div className="row">
